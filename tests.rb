...
class PaletteTests < Test::Unit::TestCase
  ...
  def test_original_indices_remain
    assert_equal(@colors_1, [@palette_1.(0), @palette_1.(0.5), @palette_1.(1)])
  end

  def test_interpolate_halfway
    assert_equal(@colors_1[1].map { |c| (c / 2.0).round }, @palette_1.(0.25))
  end

  def test_mod
    assert_equal(@colors_2[1].map { |c| (c / 2.0).round }, @palette_2.(1.5))
  end

  def test_looping
    palette = Palette.from_control_points([0, 0.5].zip(@colors_1))
    assert_equal(@colors_2[1].map { |c| (c / 2.0).round }, palette.(0.75))
  end
end
